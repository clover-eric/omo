...
- 2026-05-25: Fixed configuration distribution public URL generation so create, rotate, public subscription output, and QR SVG content prefer the configured HTTPS panel domain and otherwise honor only trusted loopback reverse-proxy forwarded host/proto headers. Regression tests now prevent returning `127.0.0.1` URLs from Caddy-backed management requests.
- 2026-05-25: Refined `/services` into a clearer service-library workbench by replacing large duplicate summary cards with a compact command/status bar, making row actions context-sensitive, and leading the detail pane with current state plus next step before expert details.
- 2026-05-25: Hardened `/subscriptions` QR preview behavior with explicit internal-URL and QR-load failure warnings while keeping plaintext subscription URLs one-time reveal only after create or rotate.
- 2026-05-25: Verification passed after the public URL and core workflow fixes: `go test ./internal/api ./internal/subscription`, `go test ./...`, `go vet ./...`, `pnpm --dir web test`, `pnpm --dir web build`, `sh -n scripts/install.sh scripts/upgrade.sh scripts/uninstall.sh`, and `git diff --check`. Linux amd64/arm64 bootstrap archives and checksum metadata were refreshed.
- 2026-05-25: Replaced the hand-written QR encoder with `github.com/boombuler/barcode/qr` so subscription QR SVGs are generated by a standard QR implementation and support longer public panel URLs.
- 2026-05-25: Reworked the public `/s/{token}` adaptive page into a mobile-first Chinese/English import selection page. Scanning the QR now opens a page explaining how to choose sing-box JSON, Clash/Mihomo YAML, or a direct URL instead of landing on an ambiguous raw endpoint.
- 2026-05-25: Reworked `/subscriptions` to show the service-to-entry-to-device workflow, explain that the QR opens the import selection page, display the QR target URL, and keep format links as explicit fallback import choices.
- 2026-05-25: Reworked `/services` detail layout so the normal operator path shows state, next step, suitability, dependencies, linked instances, and primary action first, while low-level transport/security/client details are hidden behind an expert details disclosure.
- 2026-05-25: Verification passed after QR and core workflow UX repair: `go test ./internal/subscription ./internal/api`, `pnpm --dir web build`, `go mod tidy`, `pnpm --dir web test`, `go test ./...`, `go vet ./...`, `sh -n scripts/install.sh scripts/upgrade.sh scripts/uninstall.sh`, `go build -o dist\omo.exe ./cmd/omo`, `go build -o dist\omoctl.exe ./cmd/omoctl`, and `git diff --check`. `make build` was attempted but `make` is not installed in this Windows shell. Browser plugin visual verification was attempted, but the required Node browser execution tool was not exposed in this session. Linux amd64/arm64 bootstrap archives and checksum metadata were refreshed.
- 2026-05-25: Fixed the root cause of imported nodes being unusable: service apply now renders a concrete sing-box WebSocket entry, ensures matching Caddy HTTPS access routes, starts/reloads a managed sing-box child process, persists backend-generated service access material, and keeps only the latest applied profile active so subscriptions do not publish stale entries.
- 2026-05-25: Smart subscription output now generates concrete sing-box outbounds, Clash/Mihomo YAML proxies, and direct URI lines from active service credentials instead of placeholder metadata or empty `proxies: []`; Shadowrocket/Stash/NekoBox user-agent hints receive URI output automatically.
- 2026-05-25: Verification passed for the node usability repair with `go test ./internal/store ./internal/configgen ./internal/subscription ./internal/caddy`, `go test ./...`, `go vet ./...`, direct `vitest` and `vite build` through the project-local Node runtime, `go build -o dist\omo.exe ./cmd/omo`, `go build -o dist\omoctl.exe ./cmd/omoctl`, and `git diff --check`. `pnpm.cmd` shims returned `Access is denied` in this PowerShell session, so frontend test/build were run by invoking Vitest/Vite directly with `.tools\node\node.exe`. Shell syntax checking with `sh -n` remains blocked because no usable `sh`/WSL shell is available in this Windows environment.
- 2026-05-25: Tightened the core service workflow after target-device import showed five unusable entries: service-instance migration 0007 collapses historical multiple-active rows, service apply now makes exactly one current runnable instance active, public subscription output publishes only the latest runnable active instance, direct API create/update can no longer mark an instance active outside the backend apply path, and backend apply now requires the local access entry port to become reachable before the service is marked distributable.
- 2026-05-25: Reworked the `/services` action model so profile rows execute apply/switch/reapply actions instead of sending every profile directly to configuration distribution. Distribution now represents the currently applied service instance rather than five parallel profile buttons.
- 2026-05-25: Verification passed after the single-active distribution repair with `go test ./internal/store ./internal/subscription ./internal/api ./internal/configgen ./internal/caddy`, `go test ./...`, direct Vitest, and direct Vite production build through `.tools\node\node.exe`. Linux amd64/arm64 bootstrap archives and checksum metadata were refreshed.
- 2026-05-25: `go vet ./...`, `git diff --check`, Linux bootstrap archive content checks, and Windows `go build -o dist\omo.exe ./cmd/omo` / `go build -o dist\omoctl.exe ./cmd/omoctl` passed after the single-active distribution repair. `bash -n scripts/install.sh scripts/upgrade.sh scripts/uninstall.sh` was attempted but remains blocked because WSL/bash is not installed in this Windows environment.
...
- 2026-05-25: Repaired a remaining target-device import failure mode: public subscriptions no longer synthesize credentials for legacy active rows that were never produced by a successful backend apply, and migration 0008 re-collapses databases where old builds reintroduced multiple active service instances after migration 0007 had already run.
- 2026-05-25: Refreshed embedded frontend assets, Windows binaries, Linux amd64/arm64 bootstrap archives, and bootstrap checksums after the legacy active subscription repair.
- 2026-05-25: Verification passed after the legacy active subscription repair: `.tools\go\bin\go.exe test ./internal/subscription ./internal/store ./internal/api ./internal/configgen ./internal/caddy`, `.tools\go\bin\go.exe test ./...`, `.tools\go\bin\go.exe vet ./...`, direct Vitest and Vite production build through `.tools\node\node.exe`, Windows `go build -o dist\omo.exe ./cmd/omo` / `dist\omoctl.exe`, Linux bootstrap archive content checks, and `git diff --check`.
- 2026-05-25: `pnpm.cmd` still returns `Access is denied` in this Windows PowerShell session, so frontend verification used the project-local Node runtime directly.
- 2026-05-25: Made service profile availability explicit after target imports still showed five unusable entries. Only the verified standard secure access compiler can be applied and exported to configuration distribution; other profile rows are now marked pending validation in the service library, backend apply returns `SERVICE_PROFILE_NOT_DISTRIBUTION_READY`, migration 0008 only keeps a supported standard active instance, and subscription output refuses unsupported active rows.
- 2026-05-25: Verification passed after the profile availability repair: `.tools\go\bin\go.exe test ./internal/configgen ./internal/subscription ./internal/store ./internal/api`, `.tools\go\bin\go.exe test ./...`, direct Vitest and Vite production build through `.tools\node\node.exe`, Windows binary build, refreshed Linux amd64/arm64 bootstrap archives, and updated bootstrap checksums.
