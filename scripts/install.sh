...
User=omo
Group=omo
ExecStart=${BIN_PATH} serve --addr 127.0.0.1:8080 --data ${DATA_DIR}/omo.db --caddy-config ${CADDY_CONFIG_PATH} --caddy-upstream 127.0.0.1:8080 --sing-box ${SING_BOX_BIN_PATH} --sing-box-config ${SING_BOX_CONFIG_PATH} --backup-dir ${BACKUP_DIR} --update-work-dir ${UPDATE_WORK_DIR}
Restart=on-failure
RestartSec=5s
